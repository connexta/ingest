...
@Field
def DOCKER_IMAGE_NAME = 'cnxta/ion-ingest'
@Field
def STACK = 'ingest-stack'
@Field
def NETWORK = 'cdr'
@Field
def ENVIRONMENT = [:]
...
def verifyEnvironmentVariablesExist() {
    def transformHost = System.getenv('TRANSFORM_HOST')
    if(transformHost == null || transformHost.trim().isEmpty()) {
        println 'TRANSFORM_HOST environment variable must be set'
        System.exit(1)
    }
    ENVIRONMENT.TRANSFORM_HOST = transformHost
